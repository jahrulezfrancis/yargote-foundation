...
const TimelineNode: React.FC<TimelineNodeProps> = ({ milestone, index, isActive, isVisible, isLeft }) => {
  ...
  return (
    <div
      ref={nodeRef}
      className={`timeline-item opacity-0 transform transition-all duration-700 ease-out ${isLeft ? 'translate-x-8' : '-translate-x-8'
        }`}
      ...
      <div className="block md:hidden mb-12">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-shrink-0 relative">
            <div className={`w-12 h-12 rounded-full bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center text-white font-bold text-sm shadow-lg transform transition-all duration-500 ${isActive ? 'scale-110 shadow-xl ring-4 ring-gray-200' : 'hover:scale-105'
              }`}>
          ...
          {/* Content */}
          <Card className={`flex-1 transition-all duration-500 border-l-4 border-gray-800 hover:shadow-lg ${isActive ? 'shadow-lg bg-gray-50' : 'bg-white'
            }`}>
            <CardContent className="p-6">
              <div className="space-y-3">
                ...
                <div className={`w-full h-1 bg-gray-200 rounded-full overflow-hidden`}>
                  <div className={`h-full bg-gradient-to-r from-gray-700 to-gray-900 rounded-full transition-all duration-1000 ${isActive ? 'w-full' : 'w-0'
                    }`}></div>
                </div>
              </div>
            </CardContent>
        ...
        <div className="relative">
          ...
          <div className={`grid grid-cols-12 gap-8 items-center ${isLeft ? '' : 'direction-rtl'}`}>
            ...
            <div className={`col-span-5 ${isLeft ? 'text-right' : 'text-left'}`}>
              <Card className={`transition-all duration-500 border-2 hover:shadow-xl transform hover:-translate-y-1 ${isActive
          ...
          <div className="flex items-center gap-3 bg-white rounded-full p-3 shadow-lg border border-gray-200">
            {milestones.map((milestone, index) => (
              ...
              >
                ...
                <div className={`absolute -bottom-1 left-1/2 w-1 h-1 rounded-full bg-gray-900 transform -translate-x-1/2 transition-all duration-300 ${index === activeIndex ? 'opacity-100 scale-100' : 'opacity-0 scale-0'
                  }`}></div>
              </button>
            ))}
          </div>
